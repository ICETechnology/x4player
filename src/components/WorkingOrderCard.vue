<template>
	<div class="working-order-card-ctn" v-waypoint="{active: true, callback: onWaypoint}">
		<div ref="ctn" class="flex-col tcend report-card" @click="onNameClick" v-press="onPress"
			...
			<div class="flex-row pdtb2">
				...
				<div class="cell2 flex-col" :class="`clr-${rpt.$SIDE}`">
				...
				<div class="cell3 flex-col">
					<div class="flex-1 flex-end">{{rpt.$ORDER_PRICE || $ln('市价')}}</div>
					<div class="flex-1 flex-end">{{rpt.$AVAILABLE_QTY}}</div>
				</div>
				<div class="cell4" />
			</div>
			<!-- 展開區塊 -->
