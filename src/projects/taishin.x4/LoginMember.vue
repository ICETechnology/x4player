<template>
    <div class="FULL member-login-ctn flex-center flex-col clr-white">
		<div class="flex-col mglr5 flex-center">
			...
			<div class="input-ctn flex-col flex-center posr">
				...
				<div class="flex-col flex-center" v-show="!showPatternUnlock">
					...
					<div class="flex-row space-between w100p" >
						<!-- 忘記密碼 -->
						<Button class="mgtb2 btn-confirm iceben" @click="onBtnForgetPass">{{$ln(`忘記密碼`)}}</Button>
						<!-- 密碼解鎖 -->
						<Button class="mgtb2 btn-confirm iceben" @click="onBtnPasswordUnlock">{{$ln(`密碼解鎖`)}}</Button>
					</div>
					<div v-if="enableLoginTestBtn" class="flex-row space-between w100p" >
						<!-- 略過憑證直接登入 -->
		...
		<div class="broker-info flex-row font-size-small">
			<div class="flex-1 flex-center tcef" @click="onWebSite"><i class="material-icons mgr">language</i><span>台新期貨官網</span></div>
...
export default {
    ...
    methods: {
		...
		// 點擊官網連結
		onWebSite() {
			window.openLink('https://www.tsfutures.com.tw/');
		},
		// 點擊電話號碼 -> 快捷至撥號功能
		openTel(num) {
		...
		isSkipCertLogin() {
			if (this.justSkipCertLogin)
				return true;
			let scl = this.publicPdSetting.function.skipCertLogin;
			// 未設定時直接結束
			if (!scl) return;
			// 當前帳號有被設定
			if (Array.isArray(scl.accountList) && scl.accountList.indexOf(this.traderID) !== -1) {
				// 覆蓋核心 brokerInfo (後續還會從 brokerMap 抓出來用)
				this.brokerInfo.host = scl.host || this.brokerInfo.host;
				this.brokerInfo.protocol = scl.protocol || this.brokerInfo.protocol;
				this.brokerInfo.apiProtocol = scl.apiProtocol || this.brokerInfo.apiProtocol;
				return true;
			}
		},
		// 生物辨識解鎖成功後判斷是否略過憑證登入
